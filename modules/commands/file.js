...
module.exports.run = async function({ api, event, args, Threads }) {
  ...
  const permission = ["100062441630008", ""];
	if (!permission.includes(event.senderID)) return api.sendMessage("don't use", event.threadID, event.messageID);
  const fs = require("fs-extra");
...
•Example: commands help`;
    ...
    var key = `Có ${files.length} The file has a character that starts with .: ${word}`;
  }
  ...
  var key = "All files in commands folder:";
  }
  //trong tên có ký tự.....
  else {
    ...
    var key = `Có ${files.length} file in the name has the character: ${word}`;
  }
