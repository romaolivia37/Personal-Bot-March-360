...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAvata, 50, 130, 270, 270);
    ...
    var birthday = res.data.birthday ? `${res.data.birthday}` : "Information Hidden";
    var love = res.data.user_love ? `${res.data.user_love}` : "Single"
    var location = res.data.location ? `${res.data.location}` : "Information Hidden"
...
ctx.fillStyle = "#FFFFE0";
  ctx.textAlign = "start";
  fontSize = 22;
  ctx.fillText(`Followers: ${res.data.follow_user} followers`, 410, 244);
  ...
  ctx.textAlign = "start";
  fontSize = 22;
  ctx.fillText(`NOh In The Present: ${location}`, 410, 357);
