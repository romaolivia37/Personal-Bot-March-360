    ...
    switch (args[0]) {
        case "add": {
            if (mentionID.length != 0) {
                ...
                return api.sendMessage(`[Money] Added ${money}$ cho ${message.length} người`, threadID, function () { if (error.length != 0) return api.sendMessage(`[Error] Unable to add money for ${error.length} người!`, threadID) }, messageID);
            } else {
                if (!money || isNaN(money)) return throwError(this.config.name, threadID, messageID);
                try {
                ...
                return api.sendMessage(`[Money]  ${money}$ cho bản thân`, threadID, function () { if (error.length != 0) return api.sendMessage(`[Error] Không thể thể cộng thêm tiền cho bản thân!`, threadID) }, messageID);
            }
        }
