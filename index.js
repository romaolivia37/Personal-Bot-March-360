...
for (const now of uptimelink) {
  const monitor = new Monitor({
    website: `${now}`,
    title: 'S M Bayezid Arefin',
    interval: 59,
  config: {
    intervalUnits: 'seconds'
  }
});
monitor.on('up', (res) => console.log(chalk.bold.hex("#00FF00")("[ S M Bayezid Arefin ] ❯ ") + chalk.hex("#00FF00")(`${res.website}`)))
...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "MrTomXxX.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
...
app.get('/', (req, res) => res.sendFile(__dirname+'/MrTom/MTX.html'))